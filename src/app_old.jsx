  ...
  const PaymentSummaryTab = () => {
    ...
    const loadPaymentSummary = useCallback(async () => {
      try {
        setLoading(true);
        const apiUrl = getApiUrl();
        const response = await fetch(`${apiUrl}/api/payment-summary`);
        const data = await response.json();
    ...
    const handleEditSubmit = async () => {
      try {
        const apiUrl = getApiUrl();
        const response = await fetch(`${apiUrl}/api/adjust-rent`, {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            ...
            totalRent: editForm.totalRent,
            baseRent: editForm.baseRent,
            maintenance: editForm.maintenance,
            energyCharges: editForm.energyCharges,
            gasBill: 0,
            status: editForm.status,
            allowPaidEdit: true // Allow editing paid records in Payment Summary
          }),
        });
        ...
        if (data.success) {
          ...
          setOpenSnackbar(true);
          setEditDialogOpen(false);
          loadPaymentSummary(); // Reload data
        } else {
                ...
                <TableBody>
                  {paymentSummaryData.map((row, index) => (
                    <TableRow key={index} hover>
                      ...
                      <TableCell>₹{row.baseRent?.toLocaleString()}</TableCell>
                      <TableCell>₹{row.maintenance?.toLocaleString()}</TableCell>
                      <TableCell>₹{row.energyCharges?.toLocaleString()}</TableCell>
                      <TableCell>₹{row.totalRent?.toLocaleString()}</TableCell>
                      <TableCell>
                        <Chip
                          label={row.status}
