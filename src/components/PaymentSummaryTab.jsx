...
const PaymentSummaryTab = ({ onMessage }) => {
  ...
  const loadPaymentData = useCallback(async () => {
    setLoading(true);
    try {
      const [rentResponse, tenantResponse] = await Promise.all([
        apiService.getRentData(),
        apiService.getTenantConfigs()
      ]);
      
      if (rentResponse.success && rentResponse.data && tenantResponse.success && tenantResponse.data) {
        const tenantConfigs = tenantResponse.data;
        
        // Transform the data into a flat structure for the summary table
        const summaryData = [];
        
        rentResponse.data.forEach(row => {
          const month = row.month;
          
          // Get all tenant keys from the row (excluding month and other non-tenant properties)
          const tenantKeys = Object.keys(row).filter(key => 
            ...
            !key.includes('EnergyCharges')
          );
          
          tenantKeys.forEach(tenantKey => {
            const totalRent = parseFloat(row[tenantKey]) || 0;
            const status = row[`${tenantKey}_Status`] || PAYMENT_STATUS.NOT_PAID;
            const baseRent = parseFloat(row[`${tenantKey}_BaseRent`]) || 0;
            const maintenance = parseFloat(row[`${tenantKey}_Maintenance`]) || 0;
            const energyCharges = parseFloat(row[`${tenantKey}_EnergyCharges`]) || 0;
            
            // Get tenant name from configs
            const tenantName = tenantConfigs[tenantKey]?.name || tenantKey;
