...
// import SimpleCsvEditor from './components/SimpleCsvEditor';
import {
  AppBar,
  Toolbar,
  Typography,
...
// Payment Summary Tab Component - Simple CSV Row Editor
const PaymentSummaryTab = () => {
  ...
  const handleEditSave = async () => {
    ...
    try {
      const requestData = {
        ...
        baseRent: editingField === 'baseRent' ? parseFloat(editValue) || 0 : editingRow.baseRent,
        maintenance: editingField === 'maintenance' ? parseFloat(editValue) || 0 : editingRow.maintenance,
        energyCharges: editingField === 'energyCharges' ? parseFloat(editValue) || 0 : editingRow.energyCharges,
        totalRent: editingField === 'totalRent' ? parseFloat(editValue) || 0 : editingRow.totalRent,
        gasBill: editingField === 'gasBill' ? parseFloat(editValue) || 0 : editingRow.gasBill,
        status: editingField === 'status' ? editValue : editingRow.status,
        allowPaidEdit: true
      };
      
      const data = await apiService.adjustRent(requestData);
        ...
        setPaymentSummaryData(prev => 
          prev.map(row => 
            row.id === editingRow.id 
              ? { ...row, [editingField]: editingField.includes('Rent') || editingField === 'gasBill' ? parseFloat(editValue) || 0 : editValue }
              : row
          )
        );
        handleEditCancel();
  ...
  const renderCell = (row, field, label) => {
    const isEditing = editingRow?.id === row.id && editingField === field;
        ...
        <Box display="flex" alignItems="center" gap={1}>
          {field === 'status' ? (
            ...
            <TextField
              ...
              onChange={(e) => setEditValue(e.target.value)}
              size="small"
              type={field.includes('Rent') || field === 'gasBill' ? 'number' : 'text'}
              sx={{ width: 100 }}
            />
          )}
          <Button size="small" onClick={handleEditSave} color="primary">✓</Button>
          <Button size="small" onClick={handleEditCancel} color="secondary">✗</Button>
        </Box>
      );
    }
    ...
    return (
      <Box 
        onClick={() => handleEditStart(row, field)}
        sx={{ 
          cursor: 'pointer', 
          padding: 1, 
          borderRadius: 1,
      ...
      >
        {field === 'status' ? (
          <Chip 
            label={row[field]} 
            color={row[field] === 'Paid' ? 'success' : row[field] === 'Pending' ? 'warning' : 'error'}
            size="small"
          />
        ) : field.includes('Rent') || field === 'gasBill' ? (
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" sx={{ mb: 3, fontWeight: 600 }}>
        Payment Summary - Click any cell to edit
      </Typography>
              ...
              <TableBody>
                {paymentSummaryData.map((row, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{renderCell(row, 'baseRent', 'Base Rent')}</TableCell>
                    <TableCell>{renderCell(row, 'maintenance', 'Maintenance')}</TableCell>
                    <TableCell>{renderCell(row, 'energyCharges', 'Energy')}</TableCell>
                    <TableCell>{renderCell(row, 'gasBill', 'Gas Bill')}</TableCell>
                    <TableCell>{renderCell(row, 'totalRent', 'Total Rent')}</TableCell>
                    <TableCell>{renderCell(row, 'status', 'Status')}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
...
function App() {
  ...
  const loadData = async () => {
    try {
      ...
      const rentDataResponse = await rentRes.json();
      const meterDataResponse = await meterRes.json();

      if (dashboardData.success) {
        setDashboardData(dashboardData.data);
      }
      if (rentDataResponse.success) setRentData(rentDataResponse.data);
      if (meterDataResponse.success) setMeterData(meterDataResponse.data);
    } catch (error) {
  ...
  const reloadData = async () => {
    try {
      ...
      // Reload audit data if on audit tab
      if (activeTab === 5) {
        await loadAuditData();
      }
  ...
  // Individual Tenant Details Tab - Restored Original
  const IndividualTenantDetailsTab = () => {
    ...
    // Get current month data from rentData
    const currentMonthData = rentData.find(row => row.month === selectedMonth) || {};

    // Get tenant data for the selected month and tenant
    const getTenantData = useCallback(() => {
      if (!currentMonthData || !tenantConfigs) return null;
      
      const tenantKey = selectedTenantKey;
      const tenantData = currentMonthData[tenantKey];
      const tenantConfig = tenantConfigs[tenantKey];
      
      if (!tenantData || !tenantConfig) return null;
    ...
    }, [currentMonthData, tenantConfigs, selectedTenantKey]);

    const tenantData = getTenantData();

    const handleEdit = (tenantKey) => {
      const data = currentMonthData[tenantKey];
      const config = tenantConfigs[tenantKey];
      
      setSelectedTenantForEdit(tenantKey);
      setEditForm({
        totalRent: data?.totalRent?.toString() || '0',
        baseRent: data?.baseRent?.toString() || '0',
        maintenance: data?.maintenance?.toString() || '0',
        energyCharges: data?.energyCharges?.toString() || '0',
        gasBill: data?.gasBill?.toString() || '0',
    ...
    const handleEditSubmit = async () => {
      ...
      try {
        const response = await apiService.adjustRent({
      ...
      return (
        <Box sx={{ p: 3 }}>
          ...
          <Typography variant="body1" color="text.secondary">
            No data found for {selectedTenant} in {selectedMonth}
          </Typography>
        </Box>
      );
    }
      ...
      <Box sx={{ p: 3 }}>
          ...
          <Grid item xs={12} md={6}>
            <Card>
              <CardContent>
                ...
                <Typography variant="body1">
                  <strong>Floor:</strong> {tenantData.floor}
                </Typography>
              </CardContent>
            </Card>
          </Grid>
          ...
          <Grid item xs={12} md={6}>
            <Card>
              <CardContent>
                ...
                <Typography variant="body1">
                  <strong>Base Rent:</strong> ₹{tenantData.baseRent?.toLocaleString('en-IN') || '0'}
                </Typography>
                <Typography variant="body1">
                  <strong>Maintenance:</strong> ₹{tenantData.maintenance?.toLocaleString('en-IN') || '0'}
                </Typography>
                <Typography variant="body1">
                  <strong>Energy Charges:</strong> ₹{tenantData.energyCharges?.toLocaleString('en-IN') || '0'}
                </Typography>
                <Typography variant="body1">
                  <strong>Gas Bill:</strong> ₹{tenantData.gasBill?.toLocaleString('en-IN') || '0'}
                </Typography>
                <Typography variant="h6" sx={{ mt: 2 }}>
                  <strong>Total Rent:</strong> ₹{tenantData.totalRent?.toLocaleString('en-IN') || '0'}
                </Typography>
                <Chip 
                  label={tenantData.status || 'Not Paid'} 
                  color={tenantData.status === 'Paid' ? 'success' : tenantData.status === 'Partial' ? 'warning' : 'default'}
                  sx={{ mt: 1 }}
                />
              </CardContent>
            </Card>
        ...
        <Box sx={{ mt: 3 }}>
          <Button
            variant="contained"
            startIcon={<EditIcon />}
            onClick={() => handleEdit(selectedTenantKey)}
          >
            Edit Rent Details
          </Button>
        </Box>
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  onChange={(e) => setEditForm({...editForm, totalRent: e.target.value})}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField
  ...
  // Old complex IndividualTenantDetailsTab - keeping for reference
  const OldIndividualTenantDetailsTab = () => {
    const [selectedTenantKey, setSelectedTenantKey] = useState(selectedTenant);
    const [editDialogOpen, setEditDialogOpen] = useState(false);
    const [selectedTenantForEdit, setSelectedTenantForEdit] = useState(null);
    const [editForm, setEditForm] = useState({
    ...
    const saveEditPayment = async () => {
      try {
        ...
        const result = await apiService.adjustRent({
        ...
        if (result.success) {
          ...
          setMessage(result.error || 'Error updating payment');
          setOpenSnackbar(true);
        }
      } catch (error) {
        console.error('Error updating payment:', error);
    ...
    const saveFuturePayment = async () => {
      try {
        ...
        const response = await fetch(`${getApiUrl()}/api/adjust-rent`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      <Container maxWidth="xl" sx={{ mt: { xs: 1, sm: 3 }, px: { xs: 1, sm: 2 } }}>
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3, overflowX: 'auto' }}>
          ...
          >
            ...
            <Tab icon={<ReceiptIcon />} label="Payment Summary" />
          </Tabs>
        </Box>
